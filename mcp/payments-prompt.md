...
You are wiring **billing UI** into a Next.js 15+ App Router application that uses The Bridge with **`@nebulr-group/bridge-nextjs`**. Plans and Stripe are configured elsewhere — this guide covers the frontend only: the subscription page, lifecycle notices, quota counters, the plan-selection paywall, and the billing portal. The SDK renders plans directly inside the app — there is **no handover redirect** to a separate plan-selection portal.
...
For canonical live billing state (plan + lifecycle status), read `useBridgeBilling().subscription.snapshot()` or subscribe via `useBridgeBilling().subscription.subscribe(fn)`. Register side-effect handlers with `useBridgeBilling().handle({ 'subscription.plan_changed': fn, ... })`.
