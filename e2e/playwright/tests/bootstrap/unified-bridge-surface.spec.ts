 ...
 * read surface. Ported near-verbatim from bridge-svelte; the demo exposes the
 * same Svelte-store-compatible `window.bridge` so the assertions are identical.
 *
...
test.describe('Unified bridge surface — session.snapshot end-to-end', () => {
  ...
  test('bridge.app.plans is lazy — null until .load(), populated after', async ({ page }) => {
    await page.goto('/');
    // What the first evaluate needs is `window.bridge`, so wait for that. Waiting
    // for the network to go idle would never return once the SDK holds its
    // realtime WebSocket (TBP-721).
