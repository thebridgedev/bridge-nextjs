/**
 * Per-worker Bridge apps — the thing that makes this suite parallel-safe.
 * Ported from bridge-svelte (TBP-604) for bridge-nextjs in TBP-721.
 *
 * `paymentsAutoRedirect`, `stripeEnabled`, `currency`, the SSO flags and the
 * plan catalogue are **app-level** state, and five workers drove one app:
 * welcome-paywall turned the paywall and Stripe on (and turned Stripe off
 * again in its `finally`) while subscription-flows ran its own Stripe checkout,
 * and sdk-auth-alternatives flipped Google SSO on and off under every other
 * login page. Any of those writes was visible to every worker at once.
 *
 ...
 * single-worker run is byte-for-byte the setup this suite had before.
 */
...
export function workerAppOwnerEmail(parallelIndex: number): string {
  return `iman+pw-nextjs-app-w${parallelIndex}@nebulr.group`;
}
  ...
  if (!fs.existsSync(WORKER_APPS_MANIFEST)) {
    throw new Error(
      `No per-worker app manifest at ${WORKER_APPS_MANIFEST}.\n` +
        ...
        `one of the package.json scripts (npm run test:e2e / test:e2e:stage / test:e2e:prod) ` +
