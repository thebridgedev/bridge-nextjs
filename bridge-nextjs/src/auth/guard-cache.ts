...
// Port of bridge-svelte's `auth/guard-cache.ts` (PR #56).
//
// The pending-change promise exists for a second race (TBP-654, upgrade race;
// port of bridge-svelte PR #62): the page learns about a plan change (and
// shows the new plan) a few hundred ms before the token that carries it
// arrives. A route decision taken in that window is evaluated with the old
// token and refuses the page the user can already see they have. The runtime
// registers the token refresh an authorization-affecting event starts; guards
// wait for it, bounded, before deciding.
