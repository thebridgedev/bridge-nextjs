# Live updates and the `bridge` object
...
The **`bridge` object** is the single object that exposes all of it, grouped by scope:
...
export function AccountHeader() {
  ...
  const workspace = useBridgeReadable(bridge.tenant.name);
  const subscription = useBridgeReadable(bridge.tenant.subscription);
  const user = useBridgeReadable(bridge.user);

  return (
    <>
      {branding && <img src={branding.logo} alt={branding.name} />}
...
Every field is a subscribable readable; in a component, read it with the `useBridgeReadable` hook. They are `null` until the channel delivers the first snapshot; gate on null for skeletons, or fall back to defaults. The `bridge` object's identity is stable; destructure and pass sub-references freely.
...
|------|------|---------------|
| `bridge.app.branding` | `BridgeReadable<BrandingSnapshot \| null>` | Whitelabel branding: `logo`, `name`, colors, font |
...
| `bridge.tenant.id` / `.name` | `BridgeReadable<string \| null>` | Current workspace identity |
| `bridge.tenant.subscription` | `BridgeReadable<SubscriptionSnapshot \| null>` | Canonical plan + status + endsAt (see [How billing works](/billing/how-it-works/)) |
| `bridge.tenant.entitlements` | `snapshot` readable + `can(key)` | Plan-granted capabilities, replaced live on change |
| `bridge.user` | `BridgeReadable<UserSnapshot \| null>` | Authenticated user: `id`, `email`, `role`, `tenantId` |
...
`bridge.events.handle({...})` is the one API for reacting to channel events. Use it for side effects like analytics, audit logging, or alerting (UI state updates automatically through the readables above and the drop-in components):
...
The live channel's connection state is exposed as a hook from the client entry point:
...
While the channel is down, everything keeps working from the last known state: flags evaluate from cache, readables hold their last snapshot. On reconnect the server re-sends a full `session.snapshot`, so every scope updates atomically and nothing is missed.

### Relationship to the existing hooks

The `bridge` object and the original hooks (`useAppConfig`, `useSubscription`, `useProfile`, `useAuthState`, ...) are both supported and fed by the same internal state. The `bridge` object is the newer, scoped way to read live platform state; the hooks remain the API for auth state and the classic checkout flow, covered in the [Auth](/auth/) and [How billing works](/billing/how-it-works/) guides. Use whichever fits; they don't conflict.
