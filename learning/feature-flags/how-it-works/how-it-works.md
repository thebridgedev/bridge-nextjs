...
> **Framework note:** Server Components, middleware, and route handlers
> evaluate the same flags server-side through a short-lived pull cache; see
> [Use flags on your backend](/feature-flags/using/backend/) and
> [Guard routes](/feature-flags/using/guard-routes/).
...
You can observe the connection yourself: `useRealtimeStatus()` returns the
current `ConnectionState`, which is one of `'idle'`, `'connecting'`, `'open'`,
or `'closed'`. Read it in a client component:
...
export function OfflineBanner() {
  ...
  return <div className="banner">Offline: flag values are frozen until we reconnect.</div>;
}
```
