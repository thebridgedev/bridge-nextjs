...
The subscription store is a shared, module-level store that holds the checkout-flow state. Call `loadSubscription()` to populate it.

In a client component, read it reactively with the `useSubscription()` hook:

```tsx
'use client';
import { useEffect } from 'react';
import { loadSubscription, useSubscription } from '@nebulr-group/bridge-nextjs/client';
...
export function PlanGate() {
  ...
  if (status?.shouldSelectPlan) return <p>Pick a plan to continue.</p>;
  return null;
}
```

In plain `.ts` code hooks aren't available (and a hook result read outside a component isn't reactive); take a one-off imperative read with `useBridgeStore.getState()`:

```ts
import { loadSubscription, useBridgeStore } from '@nebulr-group/bridge-nextjs/client';

await loadSubscription();

const { status, plans, loading, error } = useBridgeStore.getState().subscription;
```
...
The subscription state is shared across all components. Calling `loadSubscription()` once from a parent page is enough.
...
```ts
import { getBridgeAuth, loadSubscription } from '@nebulr-group/bridge-nextjs/client';
```
