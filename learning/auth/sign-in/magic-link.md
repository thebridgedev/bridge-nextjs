...
- **Keep the requesting route reachable** — the user is signed out when they click the link, so the route needs a `public: true` [route rule](/auth/securing/route-guards/) — and render either `MagicLink` or `LoginForm` on it. Both read `bridge_magic_link_token` on mount, exchange it for a session and strip it from the URL; there's nothing else to wire up.
