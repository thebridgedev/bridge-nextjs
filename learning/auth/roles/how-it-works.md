...
The unified `bridge` surface exposes the signed-in user's role live, reactively, via `useBridge()` + `useBridgeReadable()`:
...
For anything that must be enforced (not just hidden in the UI), check the role or a specific privilege on your backend — never rely on a frontend role check alone for access control. In Next.js specifically, that means checking it in `middleware.ts` or a Server Component/Route Handler (server-side), not only in a `'use client'` component — a client-only check like the one above can be bypassed by anyone inspecting the app's JavaScript.
