...
If you're building a custom "change this user's role" flow, handle the case where the target is the workspace's only owner. Surface a clear message instead of letting the API call fail with a generic error:
