...
A standalone SSO login button for a single federation connection (an SSO identity provider configured for your app, e.g. Google or Azure AD). Use this when you want SSO buttons outside of `LoginForm`, or to build a custom login page.
...
| `onSuccess` | `() => void` | (none) | Called after successful SSO login (popup mode only) |
| `onError` | `(error: Error) => void` | (none) | Called on error |
| `icon` | `ReactNode` | (none) | Custom icon node |
